...
#check range of Sampl_year
range(as.integer(Forest_meta$Sampl_year)) #1980 2022
...
#create Cl_time_start column - start of the temporal window
Forest_meta$Cl_time_start <- with(Forest_meta, (as.integer(Sampl_year) - (temp_width - 1)))

#create Climate_cellID column
Forest_cl_cellID <- terra::cellFromXY(object = clim_stack_proj[[1]], xy = as.matrix(Forest_meta[c('X_laea', 'Y_laea')]))

#check NA
sum(is.na(Forest_cl_cellID)) #0

Forest_meta$Climate_cellID <- Forest_cl_cellID
...
#join climate data
identical(class(Forest_meta$Climate_cellID), class(Forest_clmd$CellID)) #T
identical(class(Forest_meta$Cl_time_start), class(Forest_clmd$Cl_time_start)) #T
identical(class(Forest_meta$Sampl_year), class(Forest_clmd$Cl_time_end)) #T


Forest_meta <- dplyr::left_join(x = Forest_meta, y = Forest_clmd, by = c('Climate_cellID' = 'CellID',
                                                                                  'Cl_time_start' = 'Cl_time_start',
                                                                                  'Sampl_year' = 'Cl_time_end'))
...
#plot ids for which both Prcp and Tavg are NA
Cl_missing_loc.for <- intersect(Forest_meta$PlotID[which(is.na(Forest_meta$Prcp))], Forest_meta$PlotID[which(is.na(Forest_meta$Tavg))])

#unique for Prcp
Prcp_missing_loc.for <- setdiff(Forest_meta$PlotID[which(is.na(Forest_meta$Prcp))], Forest_meta$PlotID[which(is.na(Forest_meta$Tavg))])

#unique for Tavg
#Tavg_missing_loc.for <- setdiff(Forest_meta$PlotID[which(is.na(Forest_meta$Tavg))], Forest_meta$PlotID[which(is.na(Forest_meta$Prcp))])
...
#get info on locations missing cl data
Cl_missing_loc.for <- Forest_meta[Forest_meta$PlotID %in% Cl_missing_loc.for, c('PlotID', 'Prcp', 'Tavg', 'Cl_time_start', 'Sampl_year', 'X_laea', 'Y_laea')]
...
#fill in data
class(Forest_meta$PlotID); class(Cl_missing_loc.for$PlotID)

for(i in Cl_missing_loc.for$PlotID) {
  Forest_meta[Forest_meta$PlotID == i, 'Prcp'] <- Cl_missing_loc.for[Cl_missing_loc.for$PlotID == i, 'Prcp']
  Forest_meta[Forest_meta$PlotID == i, 'Tavg'] <- Cl_missing_loc.for[Cl_missing_loc.for$PlotID == i, 'Tavg']
}
...
#Forest_meta[Forest_meta$PlotID %in% Tavg_missing_loc.for, 'Sampl_year'] #

#get coordinates of locations for which Prcp is missing - then extract climate values for these locations

#Prcp
Prcp_missing_loc.for <- Forest_meta[Forest_meta$PlotID %in% Prcp_missing_loc.for, c('X_laea', 'Y_laea', 'PlotID', 'Sampl_year')]
...
Prcp_missing_loc.for$Start_yr <- (as.integer(Prcp_missing_loc.for$Sampl_year) - (temp_width - 1))
...
missing_over_time_prcp <- do.call(rbind, lapply(seq_len(nrow(Prcp_missing_loc.for)), function(i) {
  ...
  if(prcp_nas == temp_width) prcp_val <- NA else prcp_val <- mean(prcp_data, na.rm = T)
...
#Tavg
#Tavg_missing_loc.for <- Forest_meta[Forest_meta$PlotID %in% Tavg_missing_loc.for, c('X_laea', 'Y_laea', 'PlotID', 'Sampl_year')]
...
for(i in Prcp_missing_loc.for$PlotID) {
  Forest_meta[Forest_meta$PlotID == i, 'Prcp'] <- Prcp_missing_loc.for[Prcp_missing_loc.for$PlotID == i, 'Prcp']
}

rm(i)

#for(i in Tavg_missing_loc.for$PlotID) {
#  Forest_meta[Forest_meta$PlotID == i, 'Tavg'] <- Tavg_missing_loc.for[Tavg_missing_loc.for$PlotID == i, 'Tavg']
#}
...
#create the Topo_cellID column
Forest_topo_cellID <- cellFromXY(object = topo_stack_proj[[1]], xy = as.matrix(Forest_meta[c('X_laea', 'Y_laea')]))

#check NAs
sum(is.na(Forest_topo_cellID)) #0

#add Topo_cellID column to Forest_meta
Forest_meta$Topo_cellID <- Forest_topo_cellID

#extract topography values from xy
Forest_topo <- extract(topo_stack_proj, y = unique(Forest_meta$Topo_cellID))
...
#join topography data to Forest_meta
...
#get coordinates of locations with NAs
Forest_elev_missing <- Forest_meta[which(is.na(Forest_meta$Elevation)), c('PlotID', 'X_laea', 'Y_laea')]
...
#exclude NAs
Forest_elev_missing <- Forest_elev_missing[!is.na(Forest_elev_missing$Elevation), ] #29 values remaining (out of 78)


#same for slope and roughness
Forest_rghslo_missing <- Forest_meta[which(is.na(Forest_meta$Roughness)), c('PlotID', 'X_laea', 'Y_laea')]
...
for(i in Forest_elev_missing$PlotID) {
  ...
  Forest_meta[Forest_meta$PlotID == i, 'Elevation'] <- Forest_elev_missing[Forest_elev_missing$PlotID == i, 'Elevation']
...
#check
sum(is.na(Forest_meta$Elevation)) #49 (78 - 29) 

#Roughness and slope

for(i in Forest_rghslo_missing$PlotID) {
  
  Forest_meta[Forest_meta$PlotID == i, 'Roughness'] <- Forest_rghslo_missing[Forest_rghslo_missing$PlotID == i, 'Roughness']
  Forest_meta[Forest_meta$PlotID == i, 'Slope'] <- Forest_rghslo_missing[Forest_rghslo_missing$PlotID == i, 'Slope']
...
#create the Hmi_cellID column
Forest_hmi_cellID <- cellFromXY(object = hmi_stack_proj[[1]], xy = as.matrix(Forest_meta[c('X_laea', 'Y_laea')]))

#check NA
sum(is.na(Forest_hmi_cellID)) #0

#add Hmi_cellID to Forest_meta
Forest_meta$Hmi_cellID <- Forest_hmi_cellID

#add the Hmi_yr column, which maps Sampl_year to the closest available year of HM data
Forest_meta$Hmi_yr <- get_hmi_year(x = Forest_meta$Sampl_year) 
...
#check NA
sum(is.na(Forest_hmi_missing$Hmi_value)) #166

#exclude NAs
Forest_hmi_missing <- Forest_hmi_missing[!is.na(Forest_hmi_missing$Hmi_value), ]

#fill in hmi values in Forest_meta

for(i in Forest_hmi_missing$PlotID) {
  
  Forest_meta[Forest_meta$PlotID == i, 'Hmi_value'] <- Forest_hmi_missing[Forest_hmi_missing$PlotID == i, 'Hmi_value']
...
#check NAs
sum(is.na(Forest_meta$Hmi_value)) #166

rm(Forest_hmi_cellID, Forest_hmi, test_start, test_end, Forest_hmi_missing)

#-----save Forest_meta

#save Forest_meta - note that this object still contains NAs for the environmental drivers
save(Forest_meta, file = '/MOTIVATE/GDM_EuropeanEcoregions/tmp_obj/Forest_selection_meta.RData')
