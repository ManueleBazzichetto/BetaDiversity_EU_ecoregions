...
#For SMD:
#If balanced is not reached for a single covariate and its functions/interactions - start with std caliper = 2 for all covariates involved and progressively decrease the cal (by 0.1 std dev unit) until all covariates are below .1 abs SMD 
#If balance is not reached for a single covariate + prop score - start with caliper = 2 for both and decrease both by a 0.1 step - stop decreasing caliper when both variables (and those involved in interactions) are within
#the 0.10 threshold - if a good balance is achieved for one of the two (in terms of both main effects, interactions and functions), keep decreasing caliper only for the one that is still unbalanced
...
#Check the estimand argument (in matchit() call), which can be used to change the labeling of tr vs. control groups
#Changing the labeling doesn't mean a different estimand is actually being targeted - it's a mere trick to set labels

#--Which period is treatment (and which is control)?

#create 1/0 version of the Period column -> it is completely arbitrary which group is 1 or 0 in this case because the estimand is going to be the ATO
#setting the smallest group in terms of sample size as 1 allows more easily using the k:1 ratio, because more control units (zeros) are matched to the ones, i.e. treatment unit

#--Genetic matching - notes
...
#-------------------------------------------------function for extracting summary stats from matching object

#write a function to run several checks (weights all equal to 1, matched sample size, no replacement)
#and extract summary stats to compare the performance of different matching methods
...
#matching stats can be extracted from summary(matchit obj)
...
#from the summary()$nn object I can compare sample size of matched units when a caliper is used
...
#because these observations will be anyway excluded from the final sample used to fit the GDM
#therefore, they should not be considered for matching
env_var_nm <- c('Prcp', 'Tavg', 'Elevation', 'Roughness', 'Slope', 'Hmi_value')

all(env_var_nm %in% colnames(Grass_meta)) 

#drop NAs for environmental drivers
sapply(Grass_meta[env_var_nm], function(cl) sum(is.na(cl)))
...
#check habitat type
unique(Grass_meta$ESy_plus_LLM_lev1) #'R'

#check NAs in Sampl_year and Period
sum(is.na(Grass_meta$Sampl_year)) #0
sum(is.na(Grass_meta$Period)) #0
...
table(Grass_meta$ECO_NAME, Grass_meta$Period)

#check on remaining ecoregions
setdiff(unique(Grass_meta$ECO_NAME), Grass_eco_minN) #chr(0)
setdiff(Grass_eco_minN, unique(Grass_meta$ECO_NAME)) #chr(0)

grass_sel_econm <- setNames(Grass_eco_minN,
                            nm = c('Pan_mf', 'WesEu_bf', 'Alps_cmf', 'EuAtl_mf', 'CenEu_mf',
                                   'Carp_mf', 'Baltic_mf', 'ItaScl_sdf', 'Sar_mf', 'Celtic_bf'))

#order grass_sel_econm alphabetically
grass_sel_econm <- sort(grass_sel_econm)

#select columns for matching and add Period_bin column
Grass_sel_ecor <- lapply(grass_sel_econm, function(eco_nm) {
  
  #select grass_col_to_use
  dtf <- Grass_meta[Grass_meta$ECO_NAME == eco_nm, grass_col_to_use]
...
#check
lapply(Grass_sel_ecor, function(i) table(i[['Period_bin']]))
all(sapply(Grass_sel_ecor, function(i) isTRUE(sum(i[['Period_bin']] == 1) < sum(i[['Period_bin']] == 0)))) #TRUE

#check correlation between topographic variables before running the statistical matching

lapply(Grass_sel_ecor, function(ecor) {
  
  require(car)
  
  #with slope
  vif_vals <- vif(lm(Period_bin ~ Elevation + Roughness + Slope, data = ecor))
  
  #w/o slope
  vif_vals_noslo <- vif(lm(Period_bin ~ Elevation + Roughness, data = ecor))
  
  vif_vals_noslo <- c(vif_vals_noslo, 'Slope' = NA)
...
ggplot(Grass_meta, aes(x = Slope, y = Roughness, col = ECO_NAME)) +
  geom_point() +
...
table(Grass_sel_ecor$Alps_cmf$Period_bin)


#------check initial imbalance

Alps_cmf_init_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = NULL, distance = 'glm')
...
#logit
Alps_cmf_pscore1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = 'nearest', distance = 'glm')
...
#order
Alps_cmf_pscore1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = 'nearest',
...
Alps_cmf_pscore1_ord1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = 'nearest',
...
Alps_cmf_pscore1_ord1_ratio1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = 'nearest',
                                           distance = 'glm', m.order = 'closest', ratio = 2, std.caliper = TRUE,
                                           caliper = c(2, Elevation = 1.4, Roughness = 1.4), link = "linear.logit")

summary(Alps_cmf_pscore1_ord1_ratio1_cal1_gr, un = F, interactions = T) # ESS < Matched (SS)
plot(summary(Alps_cmf_pscore1_ord1_ratio1_cal1_gr, interactions = T, un = F))

#--Mahalanobis

Alps_cmf_mahala1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = 'nearest', distance = 'mahalanobis')
...
#order
Alps_cmf_mahala1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = 'nearest',
...
#order + ratio -> this increases precision at the expenses of balance
Alps_cmf_mahala1_ord1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = 'nearest',
...
Alps_cmf_rob_mahala1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf,
...
#order
Alps_cmf_rob_mahala1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = 'nearest',
...
#order + ratio -> this increases precision at the expenses of balance
Alps_cmf_rob_mahala1_ord1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = 'nearest',
...
#using caliper on all covariates
Alps_cmf_rob_mahala1_ord1_ratio1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf, method = 'nearest',
                                               distance = 'robust_mahalanobis', m.order = 'closest', ratio = 2, std.caliper = TRUE,
                                               caliper = c(Elevation = 1.2, Roughness = 1.2))

summary(Alps_cmf_rob_mahala1_ord1_ratio1_cal1_gr, un = F, interactions = T) # ESS < Matched (SS)
...
set.seed(grass_seeds[['Alps_cmf']])
Alps_cmf_genetic1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf,
...
#ratio
set.seed(grass_seeds[['Alps_cmf']])
Alps_cmf_genetic1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf,
...
#using caliper on all covariates
set.seed(grass_seeds[['Alps_cmf']])
Alps_cmf_genetic1_ratio1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Alps_cmf,
                                       method = 'genetic', ratio = 2, pop.size = 100, distance = 'mahalanobis',
                                       std.caliper = TRUE, caliper = c(Elevation = 0.7, Roughness = 1))

summary(Alps_cmf_genetic1_ratio1_cal1_gr, un = F, interactions = T) #
...
table(Grass_sel_ecor$Baltic_mf$Period_bin)

#------check initial imbalance

Baltic_mf_init_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = NULL, distance = 'glm')
...
#logit
Baltic_mf_pscore1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = 'nearest', distance = 'glm')
...
#order
Baltic_mf_pscore1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = 'nearest',
                                    distance = 'glm', m.order = 'closest')
...
#order + ratio -> this increases precision at the expenses of balance
Baltic_mf_pscore1_ord1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = 'nearest',
                                           distance = 'glm', m.order = 'closest', ratio = 2) #2 control units matched to 1 tr
...
Baltic_mf_mahala1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = 'nearest', distance = 'mahalanobis')
...
#order
Baltic_mf_mahala1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = 'nearest',
                                    distance = 'mahalanobis', m.order = 'closest')
...
#order + ratio -> this increases precision at the expenses of balance
Baltic_mf_mahala1_ord1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = 'nearest',
                                           distance = 'mahalanobis', m.order = 'closest', ratio = 2) #2 control units matched to 1 tr
...
#using a caliper for Elevation and Roughness
Baltic_mf_mahala1_ord1_ratio1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = 'nearest',
                                                 distance = 'mahalanobis', m.order = 'closest', ratio = 2,
                                                 std.caliper = TRUE, caliper = c(Elevation = 2, Roughness = 2))

summary(Baltic_mf_mahala1_ord1_ratio1_cal1_gr, un = F, interactions = T) # ESS < Matched (SS)
plot(summary(Baltic_mf_mahala1_ord1_ratio1_cal1_gr, interactions = T, un = F))

#--robust Mahalanobis

Baltic_mf_rob_mahala1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf,
                                   method = 'nearest', distance = 'robust_mahalanobis')
...
#order
Baltic_mf_rob_mahala1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = 'nearest',
                                        distance = 'robust_mahalanobis', m.order = 'closest')
...
#order + ratio -> this increases precision at the expenses of balance
Baltic_mf_rob_mahala1_ord1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = 'nearest',
                                               distance = 'robust_mahalanobis', m.order = 'closest', ratio = 2) #2 control units matched to 1 tr
...
Baltic_mf_rob_mahala1_ord1_ratio1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf, method = 'nearest',
                                                distance = 'robust_mahalanobis', m.order = 'closest', ratio = 2,
                                                std.caliper = TRUE, caliper = c(Elevation = 2))

summary(Baltic_mf_rob_mahala1_ord1_ratio1_cal1_gr, un = F, interactions = T) # ESS < Matched (SS)
...
Baltic_mf_genetic1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf,
                                 method = 'genetic', pop.size = 100, distance = 'mahalanobis')
...
#ratio
set.seed(grass_seeds[['Baltic_mf']])
Baltic_mf_genetic1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Baltic_mf,
                                       method = 'genetic', ratio = 2, pop.size = 100, distance = 'mahalanobis')
...
#check
table(Grass_sel_ecor$Carp_mf$Period)
table(Grass_sel_ecor$Carp_mf$Period_bin)

#------check initial imbalance

Carp_mf_init_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf, method = NULL, distance = 'glm')
...
#logit
Carp_mf_pscore1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf, method = 'nearest', distance = 'glm')
...
#order
Carp_mf_pscore1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf, method = 'nearest',
                                   distance = 'glm', m.order = 'closest')
...
Carp_mf_pscore1_ord1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf, method = 'nearest',
                                   distance = 'glm', m.order = 'closest',
                                   std.caliper = TRUE, caliper = c(2, Elevation = 2, Roughness = 2),
                                   link = "linear.logit")

summary(Carp_mf_pscore1_ord1_cal1_gr, un = F, interactions = T) #
plot(summary(Carp_mf_pscore1_ord1_cal1_gr, interactions = T, un = F))

#--Mahalanobis

Carp_mf_mahala1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf, method = 'nearest', distance = 'mahalanobis')
...
#order
Carp_mf_mahala1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf, method = 'nearest',
                                   distance = 'mahalanobis', m.order = 'closest')
...
#using caliper on all covariates
Carp_mf_mahala1_ord1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf, method = 'nearest',
                                   distance = 'mahalanobis', m.order = 'closest', std.caliper = TRUE,
                                   caliper = c(Elevation = 2, Roughness = 2))

summary(Carp_mf_mahala1_ord1_cal1_gr, un = F, interactions = T) #
plot(summary(Carp_mf_mahala1_ord1_cal1_gr, interactions = T, un = F))

#--robust Mahalanobis

Carp_mf_rob_mahala1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf,
                                  method = 'nearest', distance = 'robust_mahalanobis')
...
#order
Carp_mf_rob_mahala1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf, method = 'nearest',
                                       distance = 'robust_mahalanobis', m.order = 'closest')
...
#using caliper on all covariates
Carp_mf_rob_mahala1_ord1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf, method = 'nearest',
                                       distance = 'robust_mahalanobis', m.order = 'closest', std.caliper = TRUE,
                                       caliper = c(Elevation = 1.7, Roughness = 1.8))

summary(Carp_mf_rob_mahala1_ord1_cal1_gr, un = F, interactions = T) #
...
Carp_mf_genetic1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Carp_mf,
                               method = 'genetic', pop.size = 100, distance = 'mahalanobis')
...
summary(Celtic_bf_pscore1_ord1_ratio1_cal1_gr, un = F, interactions = T) #ESS < Matched (SS)
...
summary(Celtic_bf_mahala1_ord1_ratio1_cal1_gr, un = F, interactions = T) #ESS < Matched (SS)
...
summary(Celtic_bf_rob_mahala1_ord1_ratio1_cal1_gr, un = F, interactions = T) #ESS < Matched (SS)
...
#only genetic provides good balance and ESS = Matched SS
...
# ------ 8. Pannonian_mixed_forests - Rob. Mahalanobis ------
...
Pan_gr_perf #RMah has slightly better performance than Genetic and allows retaining more observations

# ------ 9. Sarmatic_mixed_forests - GENETIC ------

#check
table(Grass_sel_ecor$Sar_mf$Period)
table(Grass_sel_ecor$Sar_mf$Period_bin)

#------check initial imbalance

Sar_mf_init_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = NULL, distance = 'glm')
...
#logit
Sar_mf_pscore1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest', distance = 'glm')
...
#order
Sar_mf_pscore1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest',
                                    distance = 'glm', m.order = 'closest')
...
#order + ratio -> this increases precision at the expenses of balance
Sar_mf_pscore1_ord1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest',
                                           distance = 'glm', m.order = 'closest', ratio = 2) #2 control units matched to 1 tr
...
#caliper for all variables and prop score (for both SMD and Var Ratio)
Sar_mf_pscore1_ord1_ratio1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest',
                                              distance = 'glm', m.order = 'closest', ratio = 2, std.caliper = TRUE,
                                              link = 'linear.logit', caliper = c(2, Elevation = 2, Roughness = 2))

summary(Sar_mf_pscore1_ord1_ratio1_cal1_gr, un = F, interactions = T) #ESS < Matched (SS)
plot(summary(Sar_mf_pscore1_ord1_ratio1_cal1_gr, interactions = T, un = F))

#--Mahalanobis

Sar_mf_mahala1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest', distance = 'mahalanobis')
...
#order
Sar_mf_mahala1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest',
                                    distance = 'mahalanobis', m.order = 'closest')
...
#order + ratio -> this increases precision at the expenses of balance
Sar_mf_mahala1_ord1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest',
                                           distance = 'mahalanobis', m.order = 'closest', ratio = 2) #2 control units matched to 1 tr
...
#caliper for all variables (for both SMD and Var Ratio)
Sar_mf_mahala1_ord1_ratio1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest',
                                         distance = 'mahalanobis', m.order = 'closest', ratio = 2, std.caliper = TRUE,
                                         caliper = c(Elevation = 1.7, Roughness = 1.7))

summary(Sar_mf_mahala1_ord1_ratio1_cal1_gr, un = F, interactions = T) #ESS < Matched (SS)
plot(summary(Sar_mf_mahala1_ord1_ratio1_cal1_gr, interactions = T, un = F))

#--robust Mahalanobis

Sar_mf_rob_mahala1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf,
                                   method = 'nearest', distance = 'robust_mahalanobis')
...
#order
Sar_mf_rob_mahala1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest',
                                        distance = 'robust_mahalanobis', m.order = 'closest')
...
#order + ratio -> this increases precision at the expenses of balance
Sar_mf_rob_mahala1_ord1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest',
                                               distance = 'robust_mahalanobis', m.order = 'closest', ratio = 2) #2 control units matched to 1 tr
...
#caliper for all variables (for both SMD and Var Ratio)
Sar_mf_rob_mahala1_ord1_ratio1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf, method = 'nearest',
                                             distance = 'robust_mahalanobis', m.order = 'closest', ratio = 2,
                                             std.caliper = TRUE, caliper = c(Elevation = 1.5, Roughness = 1.5))

summary(Sar_mf_rob_mahala1_ord1_ratio1_cal1_gr, un = F, interactions = T) #ESS < Matched (SS)
plot(summary(Sar_mf_rob_mahala1_ord1_ratio1_cal1_gr, interactions = T, un = F))

#------genetic matching

#using GMD without prop.score
set.seed(grass_seeds[['Sar_mf']])
Sar_mf_genetic1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf,
                                method = 'genetic', pop.size = 100, distance = 'mahalanobis')
...
#ratio
set.seed(grass_seeds[['Sar_mf']])
Sar_mf_genetic1_ratio1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf,
                                       method = 'genetic', ratio = 2, pop.size = 100, distance = 'mahalanobis')
...
#caliper for all variables (for both SMD and Var Ratio)
set.seed(grass_seeds[['Sar_mf']])
Sar_mf_genetic1_ratio1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$Sar_mf,
                                     method = 'genetic', ratio = 2, pop.size = 100, distance = 'mahalanobis',
                                     std.caliper = TRUE, caliper = c(Elevation = 1.6, Roughness = 1.5))

summary(Sar_mf_genetic1_ratio1_cal1_gr, un = F, interactions = T)
plot(summary(Sar_mf_genetic1_ratio1_cal1_gr, interactions = T, un = F))

#--check matching performance

#only Genetic provides good balance and matching 2:1
Sar_gr_perf <- check_match_performance(Sar_mf_genetic1_ratio1_cal1_gr)
...
#check
table(Grass_sel_ecor$WesEu_bf$Period)
table(Grass_sel_ecor$WesEu_bf$Period_bin)

#------check initial imbalance

WesEu_bf_init_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$WesEu_bf, method = NULL, distance = 'glm')
...
#logit
WesEu_bf_pscore1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$WesEu_bf, method = 'nearest', distance = 'glm')
...
#order
WesEu_bf_pscore1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$WesEu_bf, method = 'nearest',
                                  distance = 'glm', m.order = 'closest')
...
WesEu_bf_pscore1_ord1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$WesEu_bf, method = 'nearest',
                                         distance = 'glm', m.order = 'closest',
                                         std.caliper = TRUE, caliper = c(2, Elevation = 2, Roughness = 2),
                                         link = "linear.logit")

summary(WesEu_bf_pscore1_ord1_cal1_gr, un = F, interactions = T) #
plot(summary(WesEu_bf_pscore1_ord1_cal1_gr, interactions = T, un = F))

#--Mahalanobis

WesEu_bf_mahala1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$WesEu_bf, method = 'nearest', distance = 'mahalanobis')
...
#order
WesEu_bf_mahala1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$WesEu_bf, method = 'nearest',
                                  distance = 'mahalanobis', m.order = 'closest')
...
WesEu_bf_rob_mahala1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$WesEu_bf,
                                 method = 'nearest', distance = 'robust_mahalanobis')
...
#order
WesEu_bf_rob_mahala1_ord1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$WesEu_bf, method = 'nearest',
                                      distance = 'robust_mahalanobis', m.order = 'closest')
...
WesEu_bf_rob_mahala1_ord1_cal1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$WesEu_bf, method = 'nearest',
                                        distance = 'robust_mahalanobis', m.order = 'closest',
                                        std.caliper = TRUE, caliper = c(Elevation = 1.6, Roughness = 2))
...
WesEu_bf_genetic1_gr <- matchit(formula_for_matchit, data = Grass_sel_ecor$WesEu_bf,
                              method = 'genetic', pop.size = 100, distance = 'mahalanobis')
...
sum(!forest_loc_to_drop) #1,244 (out of 109,827)
...
range(Forest_meta$Releve_area_m2) #50 1000
...
Forest_meta <- Forest_meta[Forest_meta$ECO_NAME != 'Aegean_and_Western_Turkey_sclerophyllous_and_mixed_forests', ] #106,229
...
#Alps_cmf    Carp_mf   CenEu_mf  DinMon_mf   EuAtl_mf ItaScl_sdf     Pan_mf  RodMon_mf     Sar_mf TyrAdr_smf   WesEu_bf 
#     726        823        785        233        174         33        413        567         63        194        249
...
CenEu_mf_genetic1_cal1_for <- matchit(formula_for_matchit, data = For_sel_ecor$CenEu_mf,
                                    method = 'genetic', pop.size = 100, distance = 'mahalanobis', std.caliper = TRUE,
                                    caliper = c(Elevation = 1.5, Roughness = 1.8))
...
CenEu_for_perf #Prop score has the best performance (although it has lowest sample size)
...
EuAtl_mf_pscore1_ord1_ratio1_cal1_for <- matchit(formula_for_matchit, data = For_sel_ecor$EuAtl_mf, method = 'nearest',
                                                 distance = 'glm', m.order = 'closest', ratio = 2, std.caliper = TRUE,
                                                 link = 'linear.logit', caliper = c(2, Elevation = 2, Roughness = 1.9))
...
####FROM HERE!!!!!!
#il matching è stato già completato per Pan_mf_for, ma con il dataset precedente era necessario settare ratio = 2
#mentre in questo caso non è più necessario - controllare che i codici per settare solo m.order siano corretti
#e in caso eliminare vecchi codici con ratio = 2 (ora commentati)
...
#summary(Pan_mf_pscore1_ord1_ratio1_cal1_for, un = F, interactions = T) # ESS < SS
...
summary(Pan_mf_mahala1_for, interactions = T, un = F)
...
#summary(Pan_mf_mahala1_ord1_ratio1_cal1_for, un = F, interactions = T) # ESS < SS
...
summary(Pan_mf_rob_mahala1_for, interactions = T, un = F)
...
#summary(Pan_mf_rob_mahala1_ord1_ratio1_cal1_for, un = F, interactions = T) # ESS < SS
...
summary(Pan_mf_genetic1_for, un = F, interactions = T)
...
#summary(Pan_mf_genetic1_ratio1_cal1_for, un = F, interactions = T)
#plot(summary(Pan_mf_genetic1_ratio1_cal1_for, un = F, interactions = T))
...
# ------ 8. Rodope_montane_mixed_forests - LESS THAN 1K OBS MATCHED ------
...
summary(RodMon_mf_pscore1_for, un = F, interactions = T)
plot(summary(RodMon_mf_pscore1_for, un = F, interactions = T))

#order
RodMon_mf_pscore1_ord1_for <- matchit(formula_for_matchit, data = For_sel_ecor$RodMon_mf, method = 'nearest', distance = 'glm',
                                      m.order = 'closest')

summary(RodMon_mf_pscore1_ord1_for, un = F, interactions = T)
plot(summary(RodMon_mf_pscore1_ord1_for, un = F, interactions = T))

#caliper on all variables and distance
RodMon_mf_pscore1_ord1_cal1_for <- matchit(formula_for_matchit, data = For_sel_ecor$RodMon_mf, method = 'nearest', distance = 'glm',
                                      m.order = 'closest', std.caliper = TRUE, link = 'linear.logit',
                                      caliper = c(1.2, Elevation = 1.1, Roughness = 1.7))

summary(RodMon_mf_pscore1_ord1_cal1_for, un = F, interactions = T) #less than 1K obs matched
plot(summary(RodMon_mf_pscore1_ord1_cal1_for, un = F, interactions = T))


#--Mahalanobis

RodMon_mf_mahala1_for <- matchit(formula_for_matchit, data = For_sel_ecor$RodMon_mf, method = 'nearest', distance = 'mahalanobis')

summary(RodMon_mf_mahala1_for, un = F, interactions = T)
plot(summary(RodMon_mf_mahala1_for, un = F, interactions = T))

#order
RodMon_mf_mahala1_ord1_for <- matchit(formula_for_matchit, data = For_sel_ecor$RodMon_mf, method = 'nearest', distance = 'mahalanobis',
                                      m.order = 'closest')

summary(RodMon_mf_mahala1_ord1_for, un = F, interactions = T)
plot(summary(RodMon_mf_mahala1_ord1_for, un = F, interactions = T))

#caliper on all variables
RodMon_mf_mahala1_ord1_cal1_for <- matchit(formula_for_matchit, data = For_sel_ecor$RodMon_mf, method = 'nearest', distance = 'mahalanobis',
                                      m.order = 'closest', std.caliper = TRUE, caliper = c(Elevation = 1, Roughness = 1.1))

summary(RodMon_mf_mahala1_ord1_cal1_for, un = F, interactions = T) #less than 1K obs matched
plot(summary(RodMon_mf_mahala1_ord1_cal1_for, un = F, interactions = T))


#--robust Mahalanobis

RodMon_mf_rob_mahala1_for <- matchit(formula_for_matchit, data = For_sel_ecor$RodMon_mf, method = 'nearest', distance = 'robust_mahalanobis')

summary(RodMon_mf_rob_mahala1_for, un = F, interactions = T)
plot(summary(RodMon_mf_rob_mahala1_for, un = F, interactions = T))

#order
RodMon_mf_rob_mahala1_ord1_for <- matchit(formula_for_matchit, data = For_sel_ecor$RodMon_mf, method = 'nearest', distance = 'robust_mahalanobis',
                                          m.order = 'closest')

summary(RodMon_mf_rob_mahala1_ord1_for, un = F, interactions = T)
plot(summary(RodMon_mf_rob_mahala1_ord1_for, un = F, interactions = T))

#caliper on all variables
RodMon_mf_rob_mahala1_ord1_cal1_for <- matchit(formula_for_matchit, data = For_sel_ecor$RodMon_mf, method = 'nearest', distance = 'robust_mahalanobis',
                                          m.order = 'closest', std.caliper = TRUE, caliper = c(Elevation = 1.1, Roughness = 1.1))

summary(RodMon_mf_rob_mahala1_ord1_cal1_for, un = F, interactions = T) #less than 1K obs matched
plot(summary(RodMon_mf_rob_mahala1_ord1_cal1_for, un = F, interactions = T))

#------genetic matching

#using GMD without prop.score - note that distance is set to mahalanobis so that prop score is not estimated - see examples method_genetic() 
set.seed(forest_seeds[['RodMon_mf']])
RodMon_mf_genetic1_for <- matchit(formula_for_matchit, data = For_sel_ecor$RodMon_mf,
                                   method = 'genetic', pop.size = 100, distance = 'mahalanobis')

summary(RodMon_mf_genetic1_for, un = F, interactions = T)
plot(summary(RodMon_mf_genetic1_for, un = F, interactions = T))

#caliper on Elevation
#roughness was introduced after Elevation = 1.5 because after that it started being associated with SMD > 0.1
set.seed(forest_seeds[['RodMon_mf']])
RodMon_mf_genetic1_cal1_for <- matchit(formula_for_matchit, data = For_sel_ecor$RodMon_mf,
                                  method = 'genetic', pop.size = 100, distance = 'mahalanobis',
                                  std.caliper = TRUE, caliper = c(Elevation = .3, Roughness = .9))

summary(RodMon_mf_genetic1_cal1_for, un = F, interactions = T) #less than 1K obs matched
plot(summary(RodMon_mf_genetic1_cal1_for, un = F, interactions = T))
...
summary(Sar_mf_init_for, un = F, interactions = T)
